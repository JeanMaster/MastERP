...
echo -e "${RED}🛑 Deteniendo servicios de MastERP...${NC}"

# Asegurar que estamos en el directorio raíz
PROJECT_ROOT=$(cd "$(dirname "$0")" && pwd)
cd "$PROJECT_ROOT" || exit 1

PID_FILE="$PROJECT_ROOT/.masterp.pids"
...
pkill -9 -f "node.*/MastERP/apps/backend" 2>/dev/null
pkill -9 -f "node.*/MastERP/apps/frontend" 2>/dev/null
