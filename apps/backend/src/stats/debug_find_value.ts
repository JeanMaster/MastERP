...
async function findValue() {
  ...
  returns.forEach((r) => {
    ...
    if (Math.abs(repSum - target) < tolerance) {
      console.log(
        `Found Return Replacement #${r.id} sum (${r.createdAt}): ${repSum}`,
      );
    }
    ...
    if (
      r.refundAmount &&
      Math.abs(Number(r.refundAmount) - target) < tolerance
    ) {
      console.log(
        `Found Refund Amount #${r.id} (${r.createdAt}): ${r.refundAmount}`,
      );
    }
  });
}
